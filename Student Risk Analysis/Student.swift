...
class Student{
    ...
    */
    func riskLevel()->String{
        var level:Double = self.loan.monthlyPayments/(Double(degree.avgSalary)/12)
        if(level<0.08){
            return"Low Risk"
        }
    ...
    */
    func displayComparison()->String{
        ...
        if(salDiff<0){
            return"You would be " + formatter.stringFromNumber(-salDiff)! + " safer by"
        }
        else{
            return"You would be " + formatter.stringFromNumber(salDiff)! + " safer by not"
        }
    }
